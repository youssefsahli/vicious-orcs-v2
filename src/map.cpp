...
int	glbDelusion = 0;
int	glbWasDeluded = 0;
bool	glbFinalPax = false;
bool	glbQuestActive[NUM_QUESTS];
...
{ 
    ...
    if (glbDelusion && roomType() == ROOMTYPE_VILLAGE)
    {
	if (glb_tiledefs[tile].delusion != TILE_NONE)
	    tile = (TILE_NAMES) glb_tiledefs[tile].delusion;
    }
    return tile;
}
...
FRAGMENT::buildRoom(MAP *map, ROOMTYPE_NAMES type) const
{
    ...
    room->myDepth = glb_roomtypedefs[type].depth;
	...
	for (x = 0; x < myW; x++)
	{
		...
		case '2':
		    ...
		    break;
		case '3':
		    mob = MOB::create(MOB_HEALER);
		    mob->move(room->buildPos(rx, ry));
		    tile = TILE_FLOOR;
		    break;
		case '4':
		    mob = MOB::create(MOB_SMITH);
		    mob->move(room->buildPos(rx, ry));
		    tile = TILE_FLOOR;
		    break;
		case '5':
		    mob = MOB::create(MOB_FARMER);
		    mob->move(room->buildPos(rx, ry));
		    tile = TILE_FIELD;
		    break;
		case 'A':		// any monster
		    ...
		    {
			mob = MOB::createNPC(room->getDepth());
			if (mob)
			    mob->move(room->buildPos(rx, ry));
			nmob ++;
		    }
		...
		case '(':
		    item = ITEM::createRandom(room->getDepth());
		    if (item)
		...
		case '$':
		    ...
		    item->setStackCount(rand_roll(15+room->getDepth(), 1));
		    item->move(room->buildPos(rx, ry));
		    ngold++;
		    tile = TILE_FLOOR;
		    break;
...
MAP::MAP(int depth, MOB *avatar, DISPLAY *display)
{
    ...
    for (i = 0; i < nrooms; i++)
    {
	ROOMTYPE_NAMES	roomtype = ROOMTYPE_RAT;

	switch (i)
	{
	    case 0:
		roomtype = ROOMTYPE_VILLAGE;
		break;
	}
    ...
    // Village into the cave foyer.
    ROOM::link(myRooms(0), 1,
		myRooms(1), 3);

    ROOM		*exitroom;
    int			 exitdir;

// #define SIMPLE_SQUARES

    buildSquare(ROOMTYPE_RAT, myRooms(1), 1,
				exitroom, exitdir,
#ifndef SIMPLE_SQUARES
				3, 3);
#else
				1, 1);
#endif
    buildSquare(ROOMTYPE_KOBOLD, exitroom, exitdir,
				exitroom, exitdir,
#ifndef SIMPLE_SQUARES
				4, 4);
#else
				1, 1);
#endif
    buildSquare(ROOMTYPE_SLIME, exitroom, exitdir,
				exitroom, exitdir,
#ifndef SIMPLE_SQUARES
				4, 4);
#else
				1, 1);
#endif
    buildSquare(ROOMTYPE_LICH, exitroom, exitdir,
				exitroom, exitdir,
#ifndef SIMPLE_SQUARES
				4, 4);
#else
				1, 1);
#endif
    buildSquare(ROOMTYPE_ICE, exitroom, exitdir,
				exitroom, exitdir,
#ifndef SIMPLE_SQUARES
				5, 5);
#else
    ...
    buildSquare(ROOMTYPE_PENULTIMATE, exitroom, exitdir,
				exitroom, exitdir,
				2, 2);
	...
	else
	{
	    ...
	    goal = myRooms(0)->spiralSearch(goal, 1, true);
	}
...
MAP::getNumOrcs() const
{
	...
	{
	    if (myRooms(i)->myMobs(j)->defn().delusion)
		total++;
	}
    }
    ...
    if (glbFinalPax)
	return 0;
    // Killing towns folk early would decrement the orc count, thus
    // revealing the source of the counter.  I am very torn if this is
    // a feture or a bug.
    // After some consultation I've concluded that players who figure
    // out how to kill townsfolk can be meta-rewarded with this info.
    // No doubt they'll just think it is a bug anyways :>
    // In any case, this comment serves as proof that this is not
    // an accidental oversight but a carefully deliberated decision.
...
void
MAP::triggerFinalCollapse()
{
    ROOM		*village = myRooms(0);
    ROOM		*end = myRooms(myRooms.entries()-1);

    // Fully isolate the penultimate room chain.
    for (int i = 0; i < myRooms.entries(); i++)
    {
	ROOM		*room;

	room = myRooms(i);
	if (room->type() == ROOMTYPE_PENULTIMATE)
	{
	    room->removeAllForeignPortals();
	}
	else
	{
	    room->removeAllPortalsTo(ROOMTYPE_PENULTIMATE);
	}
    }

    // Isolate the village.
    village->removeAllPortals();

    // We always link into via 1.
    ROOM::link(end, 2,
		village, 3);
    ...
    FOREACH_SPELL(spell)
    {
	totalmana += glbManaSpent[spell];
    }
    glbDelusion = totalmana;
    glbWasDeluded = totalmana;
    if (!glbDelusion)
    {
	glbFinalPax = true;

	// We don't allow people to regress.
	if (avatar())
	    avatar()->gainMP(-avatar()->getMP());
    }
    else if (getNumOrcs() > 0)
    {
	// Enrage the townsfolk
	MOBLIST		allmobs;

	getAllMobs(allmobs);
	for (int i = 0; i < allmobs.entries(); i++)
	{
	    if (allmobs(i)->defn().delusion)
		allmobs(i)->giftItem(ITEM_ENRAGED);
	}

	// Remove any stashes.
	// The townsfolk will have secured them when they heard the
	// invasion
	village->deleteAllItems();
    }
    else
    {
	// If you already killed everyone at this point, you
	// go to a normal town.
	glbDelusion = 0;
	glbWasDeluded = 0;
    }
...
#ifdef WIN32
    ofstream	os("orc.sav", ios::out | ios::binary);
#else
    ofstream	os("orc.sav");
#endif
...
MAP::load()
{
    ...
    {
#ifdef WIN32
	ifstream	is("orc.sav", ios::in | ios::binary);
#else
	ifstream	is("orc.sav");
#endif
...
MAP::MAP(istream &is)
{
    ...
    {
	is.read((char *) &val, sizeof(int32));
	glbQuestActive[quest] = val ? true : false;
    }
    myAvatar = findAvatar();
}
